...
async function healthHandler() {
  ...
  try {
    ...
    if (pingResult !== "PONG") {
      throw new Error(`Expected PONG, received: ${pingResult}`);
    }
    redisLatency = Date.now() - start;
  } catch (error: unknown) {
    redisStatus = "unhealthy";
    redisError = error instanceof Error ? error.message : "Unknown error";
    logger.error({ error }, "Health check Redis connectivity check failed");
  }

  // 3. NVIDIA NIM connection check
  ...
  const isOverallHealthy = dbStatus === "healthy" && redisStatus === "healthy" && nvidiaStatus === "healthy";
